...
const totalMountGoods = (arr) => {
  const resArr = arr.reduce((acc, item) => {
    return acc + item.price * item.count;
  }, 0);
  return resArr;
};

windowTotalMount.textContent = `$ ${totalMountGoods(goods).toLocaleString()}`;
...
list.addEventListener("click", (e) => {
  ...
  if (target.closest(".board__button_delete")) {
    ...
    windowTotalMount.textContent = `$ ${totalMountGoods(
      goods
    ).toLocaleString()}`;
